...
#### YouTube 추천 영상
- 📺 목적지 관련 YouTube 영상 5개 추천
- 이동 목적(출장/여행/맛집)에 맞는 콘텐츠
- 영상 제목, 채널, 조회수, 업로드 날짜, 재생 시간
- 클릭 시 YouTube 검색 결과로 이동

#### 네이버 블로그 추천
- 📝 목적지 관련 네이버 블로그 5개 추천
- 이동 목적에 맞는 블로그 글
- 제목, 작성자, 요약, 작성 날짜
- 클릭 시 네이버 검색 결과로 이동

#### 목적별 필터링
- 🏖️ **여행**: 관광 명소, 여행 코스, 체험 활동
- 💼 **출장**: 비즈니스 호텔, 회의 장소, 업무 팁
- 🍽️ **맛집**: 레스토랑, 카페, 음식점 후기
...
1. 계획 상세 화면 하단의 **📺 목적지 추천 콘텐츠** 섹션으로 스크롤
2. 목적 선택 (여행/출장/맛집)
3. **YouTube 추천 영상** 확인
   - 5개의 영상이 자동으로 표시됨
   - 클릭하면 YouTube에서 해당 검색 결과 확인
4. **네이버 블로그 추천** 확인
   - 5개의 블로그 글이 자동으로 표시됨
   - 클릭하면 네이버에서 해당 검색 결과 확인
5. 🔄 **새로고침** 버튼으로 콘텐츠 다시 로드
...
#### 콘텐츠 조회
```http
GET /api/destination-content/:destination?purpose={purpose}
```

**파라미터**
- `destination` (필수): 목적지 이름 (예: "제주도", "서울", "도쿄")
- `purpose` (선택): 이동 목적
  - `travel`: 여행 (기본값)
  - `business`: 출장
  - `dining`: 맛집
...
```json
{
  ...
  "data": {
    ...
    "youtube": [
      {
        "id": 1,
        "title": "제주도 여행 관광 완벽 가이드 | 꼭 가봐야 할 곳",
        "channel": "여행유튜버",
    ...
    "blogs": [
      {
        "id": 1,
        "title": "제주도 여행 관광 완벽 가이드 - 3박 4일 일정",
        "blogger": "여행블로거A",
        "summary": "제주도을(를) 다녀온 후기입니다...",
